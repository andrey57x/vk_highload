...
*   DNS Балансировка: Используется Geo-DNS + GSLB. При запросе к api.lavka.yandex.ru DNS-сервер определяет IP пользователя и возвращает IP ближайшего ДЦ
...
*  Weighted DNS: Изменение весов в GSLB позволяет плавно перенаправлять потоки пользователей в случае перегрузки одного из ДЦ (актуально для двух ДЦ в Москве)
*  Active Health Checks: Мониторим состояние каждого ДЦ
