...
| Паттерн Saga | Распределенная транзакция оформления заказа | Разбиение процесса покупки между шардами пользователей и складов на шаги. При отказе одного из шагов система выполняет компенсирующие транзакции для отката. |
