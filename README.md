...
| :--- | :--- | :--- |
| Паттерн Saga | Оформление заказа | Так как остатки складов и сами заказы лежат на разных физических серверах, мы не можем использовать обычную транзакцию в БД. Процесс разбивается на шаги: Резерв товаров на складе, Оплата, Создание заказа. Если оплата не проходит или база с заказами недоступна, система автоматически запускает обратные действия — делает запрос на возврат зарезервированных товаров обратно на полку. |
| Консистентное хеширование | Балансировка данных в Cassandra и Redis | Обычный шардинг по формуле id % N плох тем, что при добавлении нового сервера придется переносить между нодами почти все данные. Консистентное хеширование располагает все серверы на воображаемом «кольце». Данные сохраняются на ближайший сервер по часовой стрелке. Если мы добавляем новый сервер, он забирает часть данных только у одного своего соседа, а остальные серверы продолжают работать как раньше, ничего не перестраивая. |
| Gossip-протокол | Поиск упавших узлов в кластере Cassandra | В кластере Cassandra нет главного сервера, который следит за остальными. Вместо этого каждый узел раз в секунду связывается с несколькими случайными соседями и обменивается списками живых и упавших узлов. Эта информация быстро разлетается по всему кластеру, и все узлы всегда знают актуальное состояние сети. |
| Кворум | Выбор нового Master-сервера в PostgreSQL при аварии | У меня три дата-центра. Если связь между Москвой и Питером оборвется, серверы в Питере могут решить, что Москва упала, и попытаются стать Мастерами. Чтобы база не раздвоилась, используется голосование. Новым Мастером может стать только тот сервер, который наберет строго больше половины голосов кластера. В моем случае это 2 ДЦ из 3. Питерский ДЦ соберет только 1 голос, поймет, что он в изоляции, и просто будет ждать восстановления связи, защищая базу от поломки. |
