...
Как видно из расчетов, микросервисный бэкенд на Golang требует всего около 86 ядер CPU и 40 ГБ RAM. Основными потребителями серверов выступят инфраструктурные базы данных (PostgreSQL, Cassandra, Elasticsearch).
...
Для определения наиболее экономически выгодной модели проведем сравнение стоимости покупки собственного оборудования (с амортизацией на 5 лет) и аренды выделенных серверов у провайдера Hetzner.

Оборудование для покупки рассчитано в конфигураторе вендора Broadberry. Аналоги для аренды подобраны из актуальных линеек Hetzner. 
