...
Для расчета необходимого оборудования мы опираемся на метрики пиковой нагрузки (суммарно около 14 400 RPS на API) и удельное потребление ресурсов на базе открытых синтетических бенчмарков.
...
- Терминация SSL: 500 CPS на 1 ядро. При пиковых 14 400 RPS нам потребуется минимум 29 ядер CPU только на балансировщиках.
...
| DB Workers | Асинхр. запись | 0.5 | 1 | 512 Mb | 1 Gb | 9 |
...
Выбранная модель: Дешевая аренда выделенных физических серверов (Bare-Metal).
Обоснование:
1. Производительность БД: Базы данных (особенно PostgreSQL и Cassandra, принимающая 9900 RPS гео-метрик) страдают от накладных расходов виртуализации и эффекта шумных соседей на дисках. Развертывание БД напрямую на физических машинах с NVMe-накопителями гарантирует стабильно низкие latency.
2. Изоляция: В Kubernetes размещаются только stateless API-сервисы. Это уберегает кластер от задержек дискового I/O.

### 11.4 Сравнение стоимости: Покупка собственного оборудования vs Аренда

Для определения наиболее экономически выгодной модели проведем сравнение стоимости покупки собственного оборудования (с амортизацией на 5 лет) и аренды выделенных серверов (Bare-Metal) у провайдера Hetzner.

Оборудование для покупки рассчитано в конфигураторе вендора Broadberry (серия CyberServe). Аналоги для аренды подобраны из актуальных линеек Hetzner (линейки AX/EX для вычислений и БД, SX для хранилищ S3). 

Амортизация рассчитывается по формуле: Цена покупки / 60 месяцев.

| Название пула (Роль) | Конфигурация (Покупка / Аренда) | Cores | Cnt | Стоимость 1 шт (Покупка) | Амортизация в мес (Покупка) | Аренда в мес (Hetzner) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Kubernetes Cluster (Stateless сервисы) | Покупка: CyberServe SP1-102G (Xeon 16C / 128GB / 2x1TB NVMe) Аренда: Hetzner AX102 (Ryzen 16C / 128GB / 2x1.92TB NVMe) | 16 | 6 | € 5,490 | € 91 | € 145 |
| PostgreSQL DB (Профили, Заказы) | Покупка: CyberServe SP1-102G (Xeon 16C / 256GB / 4x2TB NVMe) Аренда: Hetzner EX130-R (Intel Core 24C / 256GB / 2x1.92TB NVMe) | 16-24 | 9 | € 6,850 | € 114 | € 186 |
| Cassandra (Гео-Телеметрия) | Покупка: CyberServe SP1-102G (Xeon 16C / 128GB / 4x2TB NVMe) Аренда: Hetzner AX102 (Ryzen 16C / 128GB / 2x1.92TB NVMe) | 16 | 6 | € 6,400 | € 106 | € 145 |
| Elastic + ClickHouse (Поиск, Аналитика) | Покупка: CyberServe SP1-102G (Xeon 16C / 256GB / 4x4TB NVMe) Аренда: Hetzner EX130-S (Intel Core 24C / 256GB / 2x3.84TB NVMe) | 16-24 | 6 | € 7,200 | € 120 | € 186 |
| Kafka + Redis (Очереди, In-Memory) | Покупка: CyberServe E-RS300 (Xeon 4C / 64GB / 2x1TB NVMe) Аренда: Hetzner EX44 (Intel Core 14C / 64GB / 2x512GB NVMe) | 4-14 | 6 | € 3,450 | € 57 | € 57 |
| Ceph S3 (Статика, Бэкапы) | Покупка: CyberServe E-RS300 (Xeon 4C / 32GB / 4x16TB HDD) Аренда: Hetzner SX65 (Ryzen 8C / 64GB / 4x22TB HDD) | 4-8 | 3 | € 3,900 | € 65 | € 145 |
| ИТОГО: | Всего серверов по 3 ДЦ: | | 36 шт | | € 3,465 / мес | € 5,307 / мес |

Вывод и экономическое обоснование (Что выгоднее?):

На первый взгляд, чистая амортизация купленного оборудования (3 465 € / мес) выглядит выгоднее аренды готовых серверов (5 307 € / мес). Однако такой расчет является ошибочным, так как не учитывает скрытые инфраструктурные расходы — размещение серверов в дата-центре.

1. Затраты на Colocation: 36 серверов занимают примерно 3-4 серверных шкафа с учетом резервирования питания по двум лучам. Аренда стойко-мест, оплата десятков киловатт потребляемого электричества, охлаждения и безлимитных оптических портов в 3-х независимых ДЦ обойдется компании минимум в 2 500 - 3 000 € в месяц. Прибавив это к амортизации, итоговая стоимость своего железа возрастает до ~6 465 € / мес.
2. Амортизация и риски: При покупке оборудования замена вышедших из строя накопителей полностью ложится на плечи компании. В случае аренды провайдер бесплатно меняет сгоревшее железо 24/7.
3. Гибкость: При аренде новый сервер выдается и запускается в эксплуатацию за 15 минут. Закупка, доставка через таможню и монтаж серверов Broadberry займет от 1 до 2 месяцев.

Итоговый выбор:
Для обеспечения отказоустойчивой работы проекта Яндекс Лавка с пиковой нагрузкой ~14 400 RPS экономически и операционно выгоднее арендовать выделенные серверы у хостинг-провайдера. Расчетная стоимость составит 5 307 € в месяц. Этот подход полностью удовлетворяет технические потребности микросервисов и баз данных, а также избавляет компанию от капитальных затрат на старте и непредсказуемых расходов на эксплуатацию дата-центров.
